...
# Define constants
IN_DSN <- './Gwr.gdb'
LOG_FILE <- './Results/GwrSel_SensitiveAnalysis.log'
GWR_SEL_IMAGE <- './Results/GwrSelStat_SensitiveAnalysis.RData'
GWR_SEL_EXCEL <- './Results/GwrSel_SensitiveAnalysis.xlsx'
GWR_MODEL_IMAGE <- './Results/GwrModel_SensitiveAnalysis.RData'

# Define ids
QUARTERS <- paste0('Q', 1:4)
DATA_LABELS <- c('NonLocal')
...
list_of_gwr_paras <- list(
  Q1 = list(
    ...
    NonLocal.Night = list(adapt = TRUE, method = 'cv', gweight = gwr.Gauss)
  ),
  Q2 = list(
    NonLocal.Day = list(adapt = FALSE, method = 'cv', gweight = gwr.Gauss),
    NonLocal.Night = list(adapt = TRUE, method = 'cv', gweight = gwr.Gauss)
  ),
  Q3 = list(
    NonLocal.Day = list(adapt = FALSE, method = 'cv', gweight = gwr.Gauss),
