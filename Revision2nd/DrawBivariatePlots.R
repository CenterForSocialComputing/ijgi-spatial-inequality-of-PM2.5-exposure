...
varnames <- c(
  PolDay = 'Daytime PM2.5 Concentration',
  PolNight = 'Nighttime PM2.5 Concentration',
  NonLocal = 'Non Local',
...
for (quarter in QUARTERS) {
  for (pair in var_pairs) {
    dataset <- processed_datasets[[quarter]][[pair[1]]]@data
    plots[[paste(pair[1], pair[2], quarter)]] <- 
      ...
      labs(
        title = paste(varnames[pair[2]], '~\n', varnames[pair[1]], paste0('(', quarter, ')')),
        x = expression[pair[1]], 
        y = expression[pair[2]]
      ) +
      theme_bw() +
